 ...
 */
@UnstableApi
public final class LargeSimpleCache implements Cache {

  private static final String TAG = "LargeSimpleCache";
  /**
   ...
   */
  private static final List<Character> SUBDIRECTORY_CHARACTERS = new ArrayList<>(
  ...
  /**
   * Returns whether {@code cacheFolder} is locked by a {@link LargeSimpleCache} instance. To unlock the
   * folder the {@link LargeSimpleCache} instance should be released.
   */
  ...
  public LargeSimpleCache(File cacheDir, CacheEvictor evictor) {
  ...
  @Deprecated
  @SuppressWarnings("deprecation")
  public LargeSimpleCache(File cacheDir, CacheEvictor evictor, @Nullable byte[] secretKey) {
   ...
   */
  public LargeSimpleCache(File cacheDir, CacheEvictor evictor, DatabaseProvider databaseProvider) {
    this(
        cacheDir,
        evictor,
        databaseProvider,
  ...
  /* package */ LargeSimpleCache(
    ...
    new Thread("ExoPlayer:SimpleCacheInit") {
      @Override
      public void run() {
        synchronized (LargeSimpleCache.this) {
          conditionVariable.open();
          initialize();
          LargeSimpleCache.this.evictor.onCacheInitialized();
        }
      }
    }.start();
  ...
  public synchronized File startFile(String key, long position, long length) throws CacheException {
    ...
    File cacheSubDir = new File(cacheDir, randomDirectoryName());
  ...
  private String randomDirectoryName() {
    return Character.toString(
        SUBDIRECTORY_CHARACTERS.get(random.nextInt(SUBDIRECTORY_CHARACTERS.size() - 1))) +
        SUBDIRECTORY_CHARACTERS.get(random.nextInt(SUBDIRECTORY_CHARACTERS.size() - 1));
  }
